...
const createConnection = require('./util/database');
const JobStatusContext = require('./service/job-status-context');
const FetchJobStatus = require('./service/fetch-job-status');
const UpdateJobStatus = require('./service/update-job-status');
const monitorServer = require('./service/monitor-heartbeat');
const debug = debuglog('bree');
const getRecurringJobsData = require('./service/recurring-jobs-service');
var connection;
const service = require('./service/job-service');
const ServerService = require('./service/server-service').default;
const getLeader = require('./service/get-leader');
const ServerSpecificBreeJobs = require('./model/server-specific-bree-jobs');
const ServerSpecificRecurringJobs = require('./model/server-specific-recurring-jobs');
const getServers = require('./service/get-registered-servers');
...
class Bree extends EventEmitter {
  ...
  async init() {
    debug('init');
    debug('mongodbUri', this.config.mongodbUri);
    ...
    if (this.config.mongodbUri) {
      ...
      const server = await ServerService(
        mongoose.connection,
        basename(require.main.filename) + '@' + this.config.mongodbUri
      );

      setInterval(async () => {
        await getLeader(mongoose.connection);
        monitorServer(mongoose.connection);
        getRecurringJobsData(mongoose.connection);
      }, 1000);

      await service.getJobsData(
        mongoose.connection,
        this.config.mongodbUri.maxJobs,
        this.config.jobs
      );

      const countOfRunningJobs = 0;

      setInterval(async () => {
        await service.updateFailedJobs(
          mongoose.connection,
          this.config.jobs,
          this.config.mongodbUri.retryCount
        );

        await service
          .getJobsData(mongoose.connection, this.config.mongodbUri.maxJobs)
          .then(async (jobs) => {
            jobs = jobs.filter((job) => {
              let isAlreadyProcessed = true;
              this.config.jobs.forEach((item) => {
                if (item.id === job.id) {
            ...
            const addedJobsToBree = await this.add(jobs);

            addedJobsToBree.forEach((job) => {
              if (countOfRunningJobs <= this.config.mongodbUri.maxJobs) {
                this.start(job.name);
              } else {
                addedJobsToBree.filter((jobItem) => {
                  (jobItem.status === 'completed') | 'failed' | 'skipped';
                });
              }
            });
          });
      }, this.config.mongodbUri.jobInterval);
    }
    ...
    if (
      ...
      try {
        ...
        // Error [ERR_UNSUPPORTED_ESM_URL
        // a scheme in: file, data are supported by the default ESM loader.
  ...
  async run(name) {
    const job_data1 = this.config.jobs.find((jobItem) => {
      if (jobItem.name === name) return jobItem;
    });
    const id = await job_data1.id;
    debug('run', name);
    const connection = await createConnection.createConnection(
      this.config.mysqlDatabase.driverUrl
    );
    const context = new JobStatusContext(new FetchJobStatus());
    context.setState(new FetchJobStatus());
    const jobs = await context.fetchJobs(connection);
    context.setState(new UpdateJobStatus());
    ...
    if (name) {
      ...
      this.workers.get(name).on('online', async () => {
        if (job_data1 !== null) {
          await context.updateJobStatus(connection, id, {
            status: 'running',
            startTime: new Date()
          });
        }
      ...
      this.workers.get(name).on('exit', async (code) => {
        ...
        if (level === 'error' && this.config.errorHandler) {
          ...
          this.config.jobs.filter((job) => name !== job.name);
          this.remove(name);
        }
  ...
  async start(name) {
    ...
    if (name) {
      ...
      if (job.date instanceof Date) {
        ...
        const context = new JobStatusContext(new FetchJobStatus());
        context.setState(new UpdateJobStatus());
        const job_data = this.config.jobs.find((jobItem) => {
          if (jobItem.name === name) return jobItem;
        });
        const id = await job_data.id;
  ...
  async put(job) {
    if (this.config.mysqlDatabase) {
      connection = await createConnection.createConnection(
        this.config.mysqlDatabase.driverUrl
      );
      let addedJob = await service.addJobsToDB(connection, job);

      const ServerSpecificBreeJob = ServerSpecificBreeJobs(
        connection,
        Sequelize
      );
      const ServerSpecificRecurringJob = ServerSpecificRecurringJobs(
        connection,
        Sequelize
      );
      const server = await getServers(
        connection,
        basename(require.main.filename) + '@' + this.config.mysqlDatabase.port
      );
      if (job.isRecurringJob) {
        await ServerSpecificRecurringJob.create({
          serverId: server.id,
          recurringJobId: addedJob.id
        });
      } else {
        await ServerSpecificBreeJob.create({
          serverId: server.id,
          jobId: addedJob.id
        });
      }
